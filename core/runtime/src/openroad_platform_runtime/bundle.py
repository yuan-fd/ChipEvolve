...
def _register_bytes(store: RuntimeStore, attempt_id: str, *, store_key: str,
                    kind: str, content: bytes, metadata: dict[str, Any]) -> Artifact:
    artifact = Artifact(artifact_id=_new_id("art"), kind=kind, store_key=store_key,
                        sha256=sha256(content), size_bytes=len(content), metadata=metadata)
    artifact.validate()
    with store._lock:
...
def export_run_bundle(store: RuntimeStore, run_id: str) -> dict[str, Any]:
    ...
    with zipfile.ZipFile(payload, "w", compression=zipfile.ZIP_DEFLATED) as archive:
        archive.writestr("run.json", json.dumps(view, ensure_ascii=False, sort_keys=True, indent=2))
        archive.writestr("events.json", json.dumps(
            [{"event_id": event.event_id, "run_id": event.run_id,
              ...
              "producer": event.producer, "payload": event.payload,
              "stage_run_id": event.stage_run_id, "attempt_id": event.attempt_id}
        ...
        for current_stage in view["stages"]:
            for attempt in current_stage["attempts"]:
                ...
                for filename in (
                    "adapter_request.json", "adapter_result.json", "adapter.log",
                    "runtime_input_manifest.json", "runtime_protocol_receipt.json",
                ):
                    source = workspace / filename
                    if not source.is_file():
                        continue
                    name = (
                        f"runtime/{current_stage['stage_key']}/"
                        f"attempt-{attempt['attempt_number']}/{filename}"
                    )
                    archive.writestr(name, source.read_bytes())
                    entries.append({"path": name, "kind": "runtime_file",
                ...
                for staged in attempt["inputs"]:
                    ...
                    if not source.is_file() or (
                        staged.get("sha256") and sha256(source) != staged["sha256"]
                    ):
                        raise RuntimeStoreError(
                            f"staged input {staged['destination']!r} failed hash verification"
                        )
                    name = (
                ...
                for artifact in attempt["artifacts"]:
                    ...
                    if not path.is_file() or sha256(path) != artifact["sha256"]:
                        raise RuntimeStoreError(
                            f"artifact {artifact['artifact_id']!r} failed hash verification"
                        )
                    name = (
        ...
        archive.writestr("artifacts/index.json", json.dumps(
            entries, ensure_ascii=False, sort_keys=True, indent=2,
        ))
    created = _register_bytes(
        store, latest["attempt_id"],
        store_key="runtime_bundle.zip",
        kind="runtime_bundle", content=payload.getvalue(),
        metadata={"producer": "runtime", "format": "zip", "complete": True},
    )
    return {
        "artifact_id": created.artifact_id, "kind": created.kind,
        "store_key": created.store_key, "sha256": created.sha256,
        "size_bytes": created.size_bytes, "metadata": created.metadata,
        "storage": "object", "run_id": run_id,
    }
