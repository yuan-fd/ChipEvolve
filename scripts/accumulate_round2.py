#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""P1: rerun failed 3D cases (nangate45_3D) + extra 3D platforms + 2D aes batch."""
import argparse
import json
import sys
...
def main() -> int:
    ...
    if args.mode in ("2d", "both"):
        run_2d(state, orfs["aes_cipher_top"], "aes_cipher_top",
               neighbors=6, max_parallel=args.max_parallel,
               budget_s=args.stage_budget)

    if args.mode in ("3d", "both"):
        # rerun failed nangate45_3D + add mixed platform cases
        for name in ("aes_cipher_top", "ethmac"):
            run_3d(state, orfs[name], name, "nangate45_3D")
            run_3d(state, orfs[name], name, "asap7_nangate45_3D")
