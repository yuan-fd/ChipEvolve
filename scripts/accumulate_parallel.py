#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""P1: parallel top-up — thread-pool pump for 2D campaigns + gcd 3D extra params."""
...
def run_2d_parallel(state, design_id, module, *, clock_ns, neighbors, workers):
    ...
    for member in members:
        ...
        run_ids.append((member.ordinal, run.run_id,
                        member.task_spec.parameters.get("core_utilization_pct")))
...
def main() -> int:
    ...
    run_2d_parallel(state, orfs["gcd"], "gcd", clock_ns=8, neighbors=6, workers=3)
    run_2d_parallel(state, orfs["ethmac"], "ethmac", clock_ns=8, neighbors=6, workers=3)
    run_2d_parallel(state, orfs["gcd"], "gcd", clock_ns=12, neighbors=6, workers=3)

    # 3D top-up with gcd only (other designs' 3D toolchain fails); vary util
    for util in (30, 60):
